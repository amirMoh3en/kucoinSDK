...
public class AuthenticationInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (apiKeyVersion == 1) {
            newRequestBuilder.addHeader(APIConstants.API_HEADER_PASSPHRASE.getValue(), passPhrase);
        } else if (apiKeyVersion == 2) {
            newRequestBuilder.addHeader(APIConstants.API_HEADER_PASSPHRASE.getValue(), encryptPassPhrase);
            newRequestBuilder.addHeader(APIConstants.API_HEADER_KEY_VERSION.getValue(), apiKeyVersion.toString());
        } else {
            throw new KucoinApiException("KC-API-KEY-VERSION can only be 1 and 2");
        }

        newRequestBuilder.addHeader(APIConstants.API_HEADER_KEY.getValue(), apiKey);
        newRequestBuilder.addHeader(APIConstants.API_HEADER_SIGN.getValue(), signature);
        newRequestBuilder.addHeader(APIConstants.API_HEADER_TIMESTAMP.getValue(), timestamp);
        newRequestBuilder.addHeader(APIConstants.API_HEADER_USER_AGENT.getValue(), "KuCoin-Java-SDK:" + apiKeyVersion);
